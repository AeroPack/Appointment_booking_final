When you generate an HTML artifact, do not assume it is final. You must present the artifact using npx -y lavish-axi to create a collaborative review surface. Wait for my annotations and feedback via the lavish-axi interface, read the structured TOON output to understand my requested changes, and then iterate on the HTML
